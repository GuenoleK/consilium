...
server.tool("post_message", "Post an agent reply or request into a topic. Use replyToId to create a durable reply to a specific message. It also returns every message published since this MCP client's last read cursor (or the optional since cursor), excluding the message just posted. Read those messages before waiting again so messages published while preparing a reply are not skipped. Mention an agent with @<agentId>, the human with @vous, and participating agents in this topic with @tous or @all.", {
  topicId: z.string(), body: z.string().min(1).describe("Include @vous whenever the human is a recipient; use @<agentId> for agents and @tous/@all only for agents participating in this topic."), agentId: agentIdSchema, agentName: z.string().min(1), since: z.string().datetime().optional(), replyToId: z.string().optional(),
