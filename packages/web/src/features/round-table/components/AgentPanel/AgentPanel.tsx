...
export function AgentPanel({ agents, tasks, onDisconnect, onRefreshAgents, onCreateTask, onTaskInstruction, onResolveApproval, onCancelTask }: AgentPanelProps) {
  ...
  return <aside className="agent-panel">
    <div className="agent-panel__header"><div><span>Autour de la table</span><strong>{connectedAgents.length + 1} participants connectés</strong></div><button className={refreshing ? "agent-panel__refresh agent-panel__refresh--loading" : "agent-panel__refresh"} disabled={refreshing} onClick={() => void runSpinCycle(onRefreshAgents)} aria-label="Rafraîchir les agents" title="Rafraîchir les agents"><Icon name="refresh" /></button></div>
      ...
      {visibleAgents.map((agent, index) => <div className={`agent-panel__agent agent-panel__agent--${agent.status}`} key={agent.id}>
        ...
        <div>
          ...
          <small className="agent-panel__agent-meta">
            <span>{agent.model || "Agent MCP"} · {statusLabels[agent.status]}</span>
